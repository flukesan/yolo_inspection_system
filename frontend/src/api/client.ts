...
export const refreshToken = () => api.post('/api/auth/refresh', { refresh_token: localStorage.getItem('refresh_token') });
export const getMe = () => api.get('/api/auth/me');

export interface InspectionQuery { page?: number; limit?: number; result?: string; station?: string; from?: string; to?: string; part_id?: string; }
export const createInspection = (data: Record<string, unknown>) => api.post('/api/inspection', data);
export const getInspections = (params: InspectionQuery) => api.get('/api/inspections', { params });
...
export const getPlcStatusOld = () => api.get('/api/plc/status');
export const getHealth = () => api.get('/api/health');
export const uploadImage = (formData: FormData) => api.post('/api/images/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
...
export const clearAuth = () => { localStorage.removeItem('access_token'); localStorage.removeItem('refresh_token'); localStorage.removeItem('user'); };
